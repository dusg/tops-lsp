...
bool isWindowsPath(llvm::StringRef Path) { return Path.size() > 1 && llvm::isAlpha(Path[0]) && Path[1] == ':'; }
...
public:
  llvm::Expected<std::string> getAbsolutePath(llvm::StringRef Authority, llvm::StringRef Body,
...
llvm::Expected<std::unique_ptr<URIScheme>> findSchemeByName(llvm::StringRef Scheme) {
  if (Scheme == "file")
...
bool isValidScheme(llvm::StringRef Scheme) {
  ...
  if (!llvm::isAlpha(Scheme[0]))
    return false;
  return llvm::all_of(llvm::drop_begin(Scheme, 1),
                      [](char C) { return llvm::isAlnum(C) || C == '+' || C == '.' || C == '-'; });
}
...
URI::URI(llvm::StringRef Scheme, llvm::StringRef Authority, llvm::StringRef Body)
  ...
  assert(!Scheme.empty());
  assert((Authority.empty() || Body.startswith("/")) && "URI body must start with '/' when authority is present.");
}
...
llvm::Expected<std::string> URI::resolve(llvm::StringRef FileURI, llvm::StringRef HintPath) {
...
llvm::Expected<URI> URI::create(llvm::StringRef AbsolutePath, llvm::StringRef Scheme) {
...
URI URI::create(llvm::StringRef AbsolutePath) {
  if (!llvm::sys::path::is_absolute(AbsolutePath))
    llvm_unreachable(("Not a valid absolute path: " + AbsolutePath).str().c_str());
...
llvm::Expected<std::string> URI::resolve(const URI &Uri, llvm::StringRef HintPath) {
...
llvm::Expected<std::string> URI::resolvePath(llvm::StringRef AbsPath, llvm::StringRef HintPath) {
...
URIForFile URIForFile::canonicalize(llvm::StringRef AbsPath, llvm::StringRef TUPath) {
...
llvm::Expected<URIForFile> URIForFile::fromURI(const URI &U, llvm::StringRef HintPath) {
